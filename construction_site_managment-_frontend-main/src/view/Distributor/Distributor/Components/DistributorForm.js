...
const DistributorForm = () => {
  ...
  const stateList = useSelector(
    (state) => state.distributor.data.stateList?.data
  );
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    try {
      ...
      const formattedData = {
        ...values,
        agencyData: tableData.map((item) => ({
          AGENCY_NAME: item.agencyName,
          WORKING_PERIOD: item.workingPeriod,
        })),
      };
      // //console.log("second");

      const action = edit
        ? await dispatch(putDistributor(formattedData))
        : await dispatch(postDistributor(formattedData));
  ...
  // //console.log("selectedDistributor", selectedDistributor.ID);

  useEffect(() => {
    if (edit && selectedDistributor !== null) {
      dispatch(getAgencyMAster({ DISTRIBUTOR_ID: [selectedDistributor.ID] }));
    } else {
      dispatch(getAgencyMAster());
    }
  }, [edit, selectedDistributor]);

  const data = useSelector((state) => state.distributor.data.agencyList?.data);

  // const adaptOrderDetails = (data) => {
  //   return data.map((item) => ({
  //     id: item.ID,
  //     // distributorId: item.DISTRIBUTOR_ID,
  //     agencyName: item.AGENCY_NAME,
  //     workingPeriod: item.WORKING_PERIOD,
  //   }));
  // };
  useEffect(() => {
    if (data && edit) {
          ...
          <FormikForm>
            <Row gutter={8}>
              ...
              <Col span={6}>
                ...
                >
                  <span className="text-xs">Total Salesman</span>
                  <Input
                    name="SALESMAN"
              ...
              <Col span={6}>
                ...
                >
                  <span className="text-xs">Total Delivery Boy</span>
                  <Input
                    name="DELIVERY_BOY"
              ...
              <Col span={6}>
                ...
                >
                  <span className="text-xs">Total Driver</span>
                  <Input
                    name="DRIVER"
                    placeholder="Driver"
                    value={values.DRIVER}
              ...
              <Col span={6}>
                ...
                >
                  <span className="text-xs">Total Working Area</span>
                  <Input
              ...
              <Col span={6}>
                ...
                >
                  <div className="flex flex-col mt-1.5">
                    ...
                    >
                      <Radio value="P">Partnership</Radio>
                      <Radio value="L">Limited</Radio>
                      <Radio value="PL">Private Limited</Radio>
                    </Radio.Group>
                  </div>
                </Form.Item>
              </Col>
              ...
              <Col span={5}>
                ...
                >
                  <div className="flex flex-col mt-1.5">
                    ...
                    >
                      <Radio value="T">Two Wheeler</Radio>
                      <Radio value="F">Four Wheeler</Radio>
                    </Radio.Group>
                  </div>
                </Form.Item>
              </Col>
